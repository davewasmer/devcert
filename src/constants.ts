...
const getFilteredDomains = (domains: string[]) =>
    Array.from(
        domains
            ...
            .reduce(
                (filteredList, domain) =>
                    Array.from(filteredList).reduce((matches, item) => {
...
export const getStableDomainPath = (domains: string[]) =>
    domains.length === 1 ? domains[0] : "san-" + numericHash(getFilteredDomains(domains).join(""));
...
export function withDomainSigningRequestConfig(domains: string[], cb: (filepath: string) => void) {
    ...
    let tmpFile = mktmp();
    let source = readFile(
        path.join(__dirname, "../openssl-configurations/domain-certificate-signing-requests.conf"),
        "utf-8"
    );
